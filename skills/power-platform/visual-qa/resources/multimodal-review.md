# Multimodal Review Integration

A multimodal reviewer can allow it to watch a recording (or analyze screenshots)
and compare actual behavior against the expected behavior captions.

## Prerequisites

- Optional multimodal review API key stored in `.env.local`
- A model with video/image understanding
- Collected evidence: screenshots, GIF/video recording, caption script

## Review Script (Node.js)

The example below uses Google's `@google/generative-ai` package because it is
widely available and supports image/video review. Treat it as a provider adapter:
swap the SDK and model call if the current platform uses another multimodal
reviewer.
...
const genAI = new GoogleGenerativeAI(process.env.MULTIMODAL_REVIEW_API_KEY || process.env.GEMINI_API_KEY);

async function reviewVisualTest(mediaPath, captionsPath) {
  const model = genAI.getGenerativeModel({
    model: process.env.MULTIMODAL_REVIEW_MODEL || "gemini-2.0-flash",
  });
...
When browser automation and recording tools are available, automate the full pipeline:
...
```bash
export MULTIMODAL_REVIEW_API_KEY=$(grep -E "^(MULTIMODAL_REVIEW_API_KEY|GEMINI_API_KEY)=" .env.local | head -1 | cut -d= -f2)
node tests/visual-review.mjs ./tests/recordings/test-run.gif ./tests/recordings/captions.txt
```

## Cost Considerations

- Use a low-cost multimodal model when available
