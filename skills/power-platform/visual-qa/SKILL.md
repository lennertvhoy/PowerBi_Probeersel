...
description: >
  ...
  can send the evidence to a multimodal reviewer for automated review. Catches UX misalignments, broken flows,
  missing states, and edge cases that traditional tests miss. Can use parallel agents or subagents for broader
  ...
  "catch edge cases", "multimodal review", "screen test", "ux test", "visual regression".
license: MIT
compatibility: "Any browser-capable coding agent; optional multimodal model review"
metadata:
  author: custom
  version: "1.0.0"
  platform: "Any web application"
  requires: "Browser automation or screen capture capability; optional multimodal review API key"
---
...
recording every action with structured captions, and optionally sending the evidence
to a multimodal reviewer. You catch what traditional unit/integration tests miss:
...
5. **Review the FULL evidence** — screenshots + captions + GIF/video. Not just one piece.
...
│    test run  │    │    app       │    │    evidence  │    │  reviewer    │
└─────────────┘    └──────────────┘    └─────────────┘    └──────────────┘
...
Use the browser automation tools available in the current agent runtime to walk through the app:

```
1. Navigate to the app URL (navigate)
2. Start GIF recording (start recording, if available)
3. Take initial screenshot (screenshot)
...
5. Stop GIF recording (stop recording, if available)
6. Export GIF (export recording, if available)
```
...
### Phase 5 — Multimodal Review (Optional)

If the user has a suitable multimodal API key or CLI, send the evidence for AI review.
Read `resources/multimodal-review.md` for the integration approach.
...
## Without Integrated Browser Recording
...
The user can record their own screen and provide the video plus captions to any multimodal reviewer.
