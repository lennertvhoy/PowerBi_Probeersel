...
name: plan-with-team
description: >
  Coordinates a specialist planning team to collaboratively plan Power Platform / Dataverse applications.
  Three specialists (Data Architect, UX Designer, The Skeptic) debate and refine the plan
  before any code is written. Falls back to structured single-agent planning if parallel agents or subagents
  are not available. Triggers on: "plan my app", "plan with team", "design my app",
  "architect this app", "plan the schema", "team planning", "agent team plan",
  "plan power app", "plan dataverse app", "design the data model".
license: MIT
compatibility: "Any coding agent; optional parallel agents or subagents improve planning quality"
metadata:
  author: custom
  version: "1.0.0"
  optional-capability: "parallel agents or subagents"
  platform: "Microsoft Power Platform / Dataverse"
---

# Plan With Team — Multi-Agent Planning Skill
...
5. **If parallel agents or subagents are not enabled**, fall back to single-agent structured planning
...
If the current platform supports parallel agents, subagents, workers, or delegated
tasks, use them for the Data Architect, UX Designer, and Skeptic roles.

If the current platform does not expose a safe delegation mechanism, use the
fallback in `resources/fallback-mode.md` and run the three perspectives
sequentially.
